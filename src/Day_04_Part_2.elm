...
transpose (Five (Five a1 a2 a3 a4 a5) (Five b1 b2 b3 b4 b5) (Five c1 c2 c3 c4 c5) (Five d1 d2 d3 d4 d5) (Five e1 e2 e3 e4 e5)) =
    Five
        (Five a1 b1 c1 d1 e1)
        (Five a2 b2 c2 d2 e2)
        (Five a3 b3 c3 d3 e3)
...
collectNumbers : Board -> Set Int
collectNumbers (Five a b c d e) =
    let
        numbers (Five n1 n2 n3 n4 n5) =
            Set.fromList [ n1, n2, n3, n4, n5 ]
    in
    List.foldl
        Set.union
        Set.empty
...
checkForWin : Set Int -> Board -> Bool
checkForWin markedNumbers board =
    let
        didWin (Five a b c d e) =
            Set.diff (Set.fromList [ a, b, c, d, e ]) markedNumbers == Set.empty
    in
    case ( board, transpose board ) of
        ( Five col1 col2 col3 col4 col5, Five row1 row2 row3 row4 row5 ) ->
...
solution =
    let
        boards : List Board
        boards =
            List.filterMap fromList input2

        pullNextNumber : List Int -> Set Int -> List Board -> Maybe Int -> Int
        pullNextNumber numbers markedNumbers remainingBoards lastWinningSum =
            case numbers of
                head :: tail ->
                    let
                        nextMarkedNumbers =
                            Set.insert head markedNumbers

                    in
                    case List.filter (checkForWin nextMarkedNumbers) remainingBoards of
                        winner :: winners ->
                            let
                                unmarkedNumbers =
                                    Set.diff (collectNumbers winner) nextMarkedNumbers

                                winningSum =
                                    Set.foldl (+) 0 unmarkedNumbers * head

                                nextRemainingBoards =
                                    remainingBoards
                                        |> List.filter (\board -> not <| List.member board (winner :: winners))
                            in
                            pullNextNumber tail nextMarkedNumbers nextRemainingBoards (Just winningSum)


                        [] ->
                            pullNextNumber tail nextMarkedNumbers remainingBoards lastWinningSum

                [] ->
                    case lastWinningSum of
                        Just winningSum ->
                            winningSum
    ...
    in
    pullNextNumber input Set.empty boards Nothing

fromList : List (List Int) -> Maybe Board
fromList list =
    let
        makeRows nestedList =
            case nestedList of
